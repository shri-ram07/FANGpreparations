...
const m: Module = {
  id: 'sysdesign-l3-lld-classics',
  subjectId: 'sysdesign',
  level: 3,
  title: 'LLD Classics: Parking Lot, Elevator, Splitwise, BookMyShow & Chess',
  whyItMatters:
    'The LLD round is a different exam from HLD with the same clock. Nobody asks for QPS; they ask where a responsibility lives, and then they change the requirements to see if your design survives. Five problems cover almost every LLD asked at FAANG and the Indian product companies — and each one has exactly one genuinely hard decision hiding under a pile of bookkeeping. This module teaches the method, then finds that decision in all five.',
  estMinutes: 75,
  sections: [
    {
      type: 'intuition',
      title: 'HLD asks how many machines. LLD asks how many classes.',
      md: `A city planner decides where the roads go. An architect decides where the rooms go inside one building. Same city — completely different questions.
...
- The five problems below are most of what actually gets asked. Learn the method first — the problems are just its practice range.`,
    },
    {
      type: 'intuition',
      title: 'The six-step LLD method — this is the part that transfers',
      md: `Do these in order, out loud, every time. The parking lot is disposable; the method is not.
...
6. **Write the skeleton with types, then say what you deliberately left out.** Class names, key fields, method *signatures*. Then: "I skipped payments and reservations — here is where they hook in."`,
    },
    {
      type: 'note',
      md: `**Interviewers score the discussion of tradeoffs, not the volume of code.** Nobody has ever passed a round by typing \`get_plate()\`. Enumerating every getter, setter, and constructor burns the 45 minutes you needed for the one hard decision, and it signals that you cannot tell the interesting part of a problem from the boring part. Write the fields, write the method signatures, put \`# ...\` where the obvious body goes — then spend the time you saved arguing with yourself out loud: *"I could keep pairwise balances for O(1) reads, but a corrected expense means recomputing them anyway, so the log is the truth and balances are a cache."* That sentence **is** the interview. And finish the whole skeleton before polishing any part of it: a complete rough design beats a beautiful third of one, every time.`,
    },

    {
      type: 'intuition',
      title: 'Design 1 — Parking Lot. Your turn first',
      md: `**Try it first:** list the nouns and verbs for a multi-floor parking lot, and name the one part you expect to be genuinely hard. Then read on.
...
- **The allocation policy will change** — nearest to the lift, cheapest floor, EV-first. It must be swappable without opening \`ParkingLot\`.`,
    },
    {
      type: 'intuition',
      title: 'Parking Lot — the object graph (UML-lite)',
      md: `Read the indentation as "has-a". Nothing here inherits from anything except the one interface.
...
- \`plate\` stays a string. Say "I am not making Vehicle a class — it would carry one field" and move on.`,
    },
    {
      type: 'code',
      lang: 'python',
      title: 'Parking Lot — the skeleton (runnable)',
...
if __name__ == '__main__':
    ...
    print('fee for 2h05m ->', lot.leave(car.id, 7500), '(hourly billing rounds up to 3h)')`,
      annotations: {
        13: 'The fitting rule as DATA, not branches. Adding an EV spot type is one row here — not a new if in three methods.',
        33: 'One interface, three plausible implementations, injected in the constructor. That is the honest test for "does this deserve an abstraction?"',
        45: 'Pricing is separate because it is what actually changes: weekend rates, EV surcharge, first-30-minutes-free. None of that should reopen ParkingLot.',
        54: 'The hard decision made concrete: a free list per spot type turns allocation into a pop. Scanning all 10,000 spots would be O(n) per arrival.',
        58: 'Find-then-claim is a read-modify-write. Two gates, two threads, one spot: without this, both are told spot 412 is free.',
        72: 'Freeing is just as racy as claiming — same lock. Also note the ticket keeps its spot after leaving, so the receipt still prints.',
      },
    },
    {
      type: 'note',
      md: `**The one hard decision: finding a free spot fast, and claiming it exactly once.**
...
*Claiming it.* \`find\` then \`mark taken\` is a read-modify-write, and the gap between them is where two gates hand out the same spot. The lock in \`park()\` closes that gap, and its ceiling is honest: **one lot-wide lock serialises every entry**. The upgrade path, in order — a lock **per spot type** (four locks, roughly four times the throughput), then per floor, then push the claim into the database as \`UPDATE spot SET taken=true WHERE id=? AND taken=false\` and trust the row count. That last form is the same trick BookMyShow uses below, and it is the only one that survives *multiple app servers*, which no in-process lock does. Say that limitation before the interviewer asks.`,
    },

    {
      type: 'intuition',
      title: 'Design 2 — Elevator. Your turn first',
      md: `**Try it first:** there are two kinds of request in this system. Name them, and say why the difference changes the design. Then read on.
...
- Cars are physical objects. Direction reversals cost time and electricity, so a good algorithm minimises them.`,
    },
    {
      type: 'note',
      md: `**Why FCFS is wrong, and what SCAN/LOOK is.** Serve requests in arrival order, and a car sitting on floor 5 with a pending call for floor 6 will first drive all the way down to floor 1 — because that button was pressed thirty seconds earlier — then climb back past 5 to reach 6. Riders experience that as the elevator ignoring them. **SCAN** fixes it: keep travelling in one direction, stop at every requested floor on the way, and only reverse at the end of the shaft. **LOOK** is SCAN with the obvious improvement — reverse as soon as nothing further remains in the current direction, instead of driving to the top floor out of principle. Every real elevator runs LOOK. This is literally **disk-head scheduling with doors**, and saying so is exactly the kind of transfer interviewers listen for. The cost to admit: pure LOOK can starve a request if same-direction calls keep arriving ahead of it, which is why production dispatchers add an age term to the score.`,
    },
    {
      type: 'intuition',
      title: 'Elevator — the object graph (UML-lite)',
      md: `- **Dispatcher** — owns the bank of cars and routes every hall call
...
- **Button**, **Door**, and **Display** are nouns you should mention and then cut — they are I/O, not design.`,
    },
    {
      type: 'code',
      lang: 'python',
      title: 'Elevator — LOOK scheduling plus a multi-car dispatcher (runnable)',
...
if __name__ == '__main__':
    ...
    print('A floors (* = doors open):', ' '.join(trace))`,
      annotations: {
        6: 'The four states. IDLE to MOVING on a request, MOVING to DOORS_OPEN on arrival, DOORS_OPEN back to IDLE after the dwell tick — that is the entire machine.',
        17: 'Both request kinds collapse to one line. The difference between them lives in the dispatcher, not the car.',
        30: 'LOOK in one expression: which pending stops are ahead of me, given my signed direction. FCFS would sort by arrival time here and yo-yo the car.',
        31: 'The LOOK turn: reverse the moment nothing remains this way. SCAN would keep going to the end of the shaft first.',
        43: 'Multi-car dispatch in one line — score every car, give the call to the cheapest. Everything genuinely hard is inside cost().',
        55: 'The wrong-direction penalty. Tuning these three constants IS tuning the product; add an age term here to stop starvation.',
      },
    },
    {
      type: 'note',
      md: `Run output: the call on floor 6 goes to car **A** — six floors away and climbing — not to car **B**, which is one floor away but descending. B would have to finish its trip to the lobby and come back. A's trace is \`2 3 4 5 6 6* 7 8 9 9*\`: it picks up floor 6 for free on the way to 9, which is the entire point of LOOK.
...
Two alternatives to the greedy score, worth naming as "what I would reach for next": **zoning** (cars 1–2 own floors 1–10, cars 3–4 own 11–20 — trivial to implement, terrible when one zone empties) and **full assignment optimisation** (re-solve all pending calls whenever one arrives — better results, far more code, and the reason destination-dispatch lobbies make you enter your floor *before* boarding).`,
    },

    {
      type: 'intuition',
      title: 'Design 3 — Splitwise. Your turn first',
      md: `**Try it first:** three friends, four expenses, some split unevenly. What is the *smallest* thing you must store, and what can you always recompute from it? Then read on.
...
- An expense can be **edited or deleted months later**, and every balance it touched must correct itself.`,
    },
    {
      type: 'intuition',
      title: 'Splitwise — the object graph (UML-lite)',
      md: `- **Ledger** (one per group) — the aggregate root; owns an **append-only expense log**
...
- The pairwise "A owes B ₹300" table everyone reaches for first is deliberately **not stored** — it is a projection of the log, and the note below is the argument for why.`,
    },
    {
      type: 'code',
      lang: 'python',
      title: 'Splitwise — split strategies, the balance sheet, and real debt simplification (runnable)',
      code: `from abc import ABC, abstractmethod
...
if __name__ == '__main__':
    ...
    print('transfers:', simplify(led.net()), '-> 2 payments instead of 4')`,
      annotations: {
        10: 'Integers, always. 0.1 + 0.2 != 0.3 in floats, and a money bug that drifts one paise per expense is a support ticket you cannot reproduce.',
        11: 'The remainder problem, solved in one line: 10000 paise across 3 people is 3334/3333/3333. Interviewers ask who gets the extra paise — have an answer.',
        29: 'Percent has the same crumb problem, one layer deeper. Assign the leftover deterministically or the group total silently stops matching.',
        49: 'The entire balance sheet is this loop: everyone owes their share, the payer is credited the full amount. Sum of all values is always exactly zero.',
        57: 'A free function, not a Ledger method — it depends only on the net map. Resisting the urge to make everything a method is a real design signal.',
        67: 'Each iteration zeroes at least one person, so it terminates in at most n-1 transfers. Not guaranteed minimal - say so out loud.',
      },
    },
    {
      type: 'note',
      md: `Run output: after a ₹300 dinner paid by Asha and a ₹90 taxi paid by Bilal, the net is \`Asha +17000, Bilal -4000, Chen -13000\` paise, and \`simplify\` turns four pairwise IOUs into **two** transfers.
...
**On simplification.** This is a genuine algorithm question wearing a product costume. Net every user to one number, then greedily match the biggest debtor to the biggest creditor. It finishes in at most **n − 1** transfers for n people with non-zero balances, because each step zeroes at least one of them. It is **not guaranteed minimal** — the true minimum requires finding subsets that sum to zero, which is NP-hard — and volunteering that, instead of claiming optimality, is what separates a real answer from a memorised one. Product wrinkle worth adding: simplification can leave you owing money to someone you never transacted with, which users find alarming, so real apps make it opt-in per group.`,
    ...
    {
      type: 'intuition',
      title: 'Design 4 — BookMyShow. Your turn first',
      md: `**Try it first:** two people tap the same seat 40 ms apart. Write down exactly what your code does between "read the seat" and "write the seat" — and what happens if the other request lands inside that gap. Then read on.
...
- Reads (browsing seat maps) outnumber writes enormously, and a slightly stale seat map is survivable — a double booking is not.`,
    },
    {
      type: 'visual',
      component: 'PointerBoxDiagram',
      props: {
        title: 'One seat, two users: the double-booking race and the one-line fix',
        notice: 'Watch the gap between reading the seat and writing it. Every seat-locking design is an argument about that gap.',
        leftLabel: 'requests',
        rightLabel: 'seat row in the DB',
        frames: [
          {
            note: 'Show 7, seat A5 is free. Two phones tap it 40 ms apart. Both app servers run the same code: read the seat, see "free", then write. Right now both reads have returned free — and neither request has any idea the other exists.',
            stack: [
              { name: 'asha: SELECT A5', to: 'a5' },
              { name: 'bilal: SELECT A5', to: 'a5' },
            ],
            heap: [{ id: 'a5', value: 'state=free, holder=null', label: 'both reads see the same truth' }],
          },
          {
            note: 'Asha\'s server writes first: UPDATE seat SET state=booked, holder=asha WHERE seat_id=A5. It succeeds. Nothing is wrong yet — the row WAS free when she read it, and no one has told her otherwise.',
            stack: [
              { name: 'asha: UPDATE -> booked', to: 'a5' },
              { name: 'bilal: still holds a stale read', value: 'saw free' },
            ],
            heap: [{ id: 'a5', value: 'state=booked, holder=asha', label: 'Asha has the seat' }],
          },
          {
            note: 'Bilal\'s write lands 4 ms later. His UPDATE names the seat but says nothing about its current state, so the database does exactly as asked and overwrites Asha. One row, one holder — and two people with confirmation emails and two charged cards.',
            stack: [{ name: 'bilal: UPDATE -> booked', to: 'a5', danger: true }],
            heap: [
              { id: 'a5', value: 'state=booked, holder=bilal', label: 'DOUBLE BOOKED - Asha paid too', danger: true },
            ],
          },
          {
            note: 'The fix is not a bigger lock or a faster read. It is moving the check INSIDE the write: UPDATE ... SET state=held, holder=asha, expires=now+10min WHERE seat_id=A5 AND state=free. The condition and the write are one atomic statement, so no gap exists to lose. Asha matches one row: rowcount 1.',
            stack: [{ name: 'asha: UPDATE ... WHERE state=free', to: 'a5' }],
            heap: [
              { id: 'a5', value: 'state=held, holder=asha, expires=T+10m', label: 'rowcount 1 -> hold granted' },
            ],
          },
          {
            note: 'Bilal runs the identical statement. The WHERE clause no longer matches anything: rowcount 0. His app reads that as "seat gone", greys it out, and suggests A6. Nobody was locked, polled, or queued. Payment then flips held to booked; if payment never arrives, expires does the cleanup with no background job needed on the hot path.',
            stack: [{ name: 'bilal: UPDATE ... WHERE state=free', to: 'a5', danger: true }],
            heap: [
              { id: 'a5', value: 'state=held, holder=asha', label: 'rowcount 0 -> Bilal rejected, cleanly' },
            ],
          },
        ],
      },
    ...
    {
      type: 'intuition',
      title: 'BookMyShow — the object graph (UML-lite)',
      md: `- **Movie → Show → Screen** — the static catalogue, and the boring 10% of this problem
...
- Seats are **not** objects with a \`book()\` method. Availability is per-show, so the state belongs on the (show, seat) row — a classic noun that is really a *relationship*. Getting this wrong is the most common failure in this problem.`,
    },
    {
      type: 'code',
      lang: 'python',
      title: 'BookMyShow — hold, pay, confirm (runnable)',
...
if __name__ == '__main__':
    ...
    print('asha pays at 700s ->', inv.confirm(a, 700), '- hold lapsed, refund path')`,
      annotations: {
        23: 'The schema decision that makes everything else easy: state lives on (show, seat), not on a Seat object. Two shows on one screen share the layout, never the availability.',
        43: 'An expired hold is as good as free. Encoding that here means no background sweeper is needed on the booking path - expiry is lazy, and correct.',
        45: 'The line to write on the whiteboard. Condition and write in one statement, then trust rowcount. This is what actually stops the double booking.',
        51: 'All-or-nothing across the seat set: a group booking that half-succeeds is worse than one that fails. In SQL this is the rowcount check plus ROLLBACK.',
        60: 'Idempotency, from the Level 1 resilience module: gateways retry webhooks, so confirm() must be safe to call twice and return the same answer.',
        62: 'The genuinely nasty case: payment succeeded, hold expired, seat re-sold. There is no clever fix - there is only a refund and an honest message.',
      },
    },
    {
      type: 'note',
      md: `**The one hard decision: how long is a hold, and what happens when payment lands after it expires?**
...
**Deliberately left out:** the seat *map* read path (cache it hard — a slightly stale map is fine because the conditional update is the real gate), dynamic pricing, and the waiting-room queue that a 200,000-tap on-sale actually needs. Name them as the next things you would build.`,
    },

    {
      type: 'intuition',
      title: 'Design 5 — Chess. Your turn first',
      md: `**Try it first:** where does the knowledge *"this move is illegal because it leaves my own king in check"* live — on the Knight, on the Board, or somewhere else? Commit to an answer before reading on.
...
- Adding a variant rule must not mean editing six piece classes.`,
    },
    {
      type: 'intuition',
      title: 'Chess — the object graph (UML-lite)',
      md: `- **Board** — owns the squares and the history; every rule that needs the **whole position** lives here
...
- Notice the split before you see the code: **shape** is per-piece and local; **legality** is positional and global. That one boundary is the answer to the question above.`,
    },
    {
      type: 'code',
      lang: 'python',
      title: 'Chess — pseudo-moves on pieces, legality on the board (runnable)',
      code: `from abc import ABC, abstractmethod
from dataclasses import dataclass
from enum import Enum
...
if __name__ == '__main__':
    ...
    print('white in check?', b.in_check(Color.WHITE), '- no, but the bishop is pinned to its king')`,
      annotations: {
        16: 'The abstract method is deliberately narrow: geometry only. The moment it needs history or the king, the abstraction has leaked and the design is wrong.',
        19: 'One sliding helper covers rook, bishop, queen and (with limit=1) king and knight. Six piece classes do not need six algorithms.',
        46: 'Move is the undo record, not a command object. was_first restores the moved flag - forget it and castling silently becomes legal after an undo.',
        59: 'Make/unmake instead of copy. Copying the 64-square board per candidate move is correct and roughly 20x slower; engines and interviews both use this.',
        74: 'in_check asks the opponent pieces where they can go. It reuses pseudo(), which is exactly why pseudo must NOT already filter for check - that would recurse forever.',
        79: 'The whole design in three lines: apply, ask, undo. A pinned piece has pseudo-moves and zero legal ones, and this loop is where that difference appears.',
      },
    },
    {
      type: 'note',
      md: `Run output: the bishop has **9 pseudo-moves and 0 legal moves** — it is pinned to its own king by the black rook — while white is not in check at all. No piece can compute that from its own geometry. That is the proof.
...
**Deliberately left out: the Pawn.** It is the special-case pile — one step, two steps from home, diagonal-only capture, en passant, promotion to four different pieces — and it is precisely where every naive design finally breaks. Say that out loud instead of pretending it is one more \`ray()\` call. Also left out: checkmate (\`in_check(c) and no legal moves anywhere for c\`), stalemate (the same with \`not in_check\`), and the draw rules that need the history you already have.`,
    },

    {
      type: 'intuition',
      title: 'Snake & Ladder — the easy one, in five bullets',
      md: `It turns up as a warm-up or a 20-minute screen. The only way to fail it is to over-model it.
...
- **The hard decision, such as it is:** where does a jump get applied — inside \`play()\` or on \`Board.resolve(pos)\`? Put it on the board, so a variant with *chained* jumps changes exactly one method.`,
    },
    {
      type: 'code',
      lang: 'python',
      title: 'Snake & Ladder — injectable dice makes it testable (runnable)',
...
if __name__ == '__main__':
    ...
    print('winner:', play(board, ['Asha', 'Bilal'], d))`,
      annotations: {
        5: 'A Protocol, not an ABC: structural typing means RealDice and ScriptedDice need no base class, and a lambda-ish stub works in tests too.',
        11: 'The entire testability argument. With this class the game is a pure function of a roll list, so "does a snake at 87 work?" becomes a one-line assertion.',
        16: 'One dict for both snakes and ladders. Two classes here would be identical fields with different names - the most common over-modelling in this problem.',
        24: 'The exact-landing rule, in one branch. Whether it should exist at all is a clarifying question, not an assumption.',
      },
    },
  ],
  quiz: [
    {
      question: 'Ten minutes left in an LLD round. Your ParkingLot has no pricing logic yet, and your Ticket class has no getters. What do you do?',
      options: [
        {
          text: 'Write the getters and setters — a complete class is expected',
          explanation: 'Nobody has ever scored a point on get_plate(). You would spend the last ten minutes on the least informative code in the design.',
        },
        {
          text: 'Sketch the Pricing skeleton, then say out loud what you deliberately left out',
          explanation: 'Correct. A complete rough design beats a beautifully finished third of one, and naming the gaps proves the gaps were choices.',
        },
        {
          text: 'Refactor Spot into an interface so future spot types are easy',
          explanation: 'A speculative abstraction with one implementation — precisely the instinct this round is testing you to suppress.',
        },
        {
          text: 'Add input validation to every method',
          explanation: 'Real in production, near-zero signal in a 45-minute design round. Mention it in one sentence and move on.',
        },
      ],
      correct: 1,
    },
    {
      question: 'A car is at floor 5 heading up. Pending: floor 6 (pressed just now) and floor 1 (pressed 30 seconds ago). What does FCFS do, and why is it wrong?',
      options: [
        {
          text: 'Serves 6 then 1 — FCFS is fine here',
          explanation: 'FCFS means arrival order. Floor 1 was pressed first, so FCFS serves it first — that is the whole problem.',
        },
        {
          text: 'Serves 1 then 6, which is optimal because it clears the oldest request',
          explanation: 'It does serve 1 first, but calling that optimal misses the cost: the car drives past floor 6 twice while a rider stands there.',
        },
        {
          text: 'Serves 1 first — driving down past floor 6 and back up. LOOK fixes it by finishing the current direction first',
          explanation: 'Correct. The wasted double traversal is what riders read as "the elevator is ignoring me". LOOK serves 6 on the way, then reverses for 1.',
        },
        {
          text: 'Refuses the floor-6 request until the car is idle',
          explanation: 'No scheduler drops requests. The question is ordering, not admission.',
        },
      ],
      correct: 2,
    },
    {
      question: 'Your Splitwise clone stores a pairwise balance table updated on every expense. A user edits an expense from three months ago. What is the real risk?',
      options: [
        {
          text: 'The balance table can drift from the expense log with no way to tell which is right — because the balance is the only place that number lives',
          explanation: 'Correct. One bug in the inverse-delta path leaves balances that correspond to no set of expenses, and nothing to reconcile against.',
        },
        {
          text: 'The edit will be slow',
          explanation: 'It is a handful of row updates. Speed is not what breaks here.',
        },
        {
          text: 'Nothing — the update path handles it',
          explanation: 'It handles the happy path. The risk is what a bug in that path leaves behind permanently, with no source of truth to recover from.',
        },
        {
          text: 'Currency rounding errors',
          explanation: 'A real concern, but it exists in both designs. It is not what editing an old expense specifically breaks.',
        },
      ],
      correct: 0,
    },
    {
      question: 'Two requests read seat A5 as free, then both run UPDATE seat SET state=\'booked\' WHERE seat_id=\'A5\'. What actually prevents the double booking?',
      options: [
        {
          text: 'Making the read and the write faster',
          explanation: 'That narrows the window without closing it. A race you can still lose in 2 ms is still a race — and at on-sale traffic you will lose it.',
        },
        {
          text: 'A mutex in the application server',
          explanation: 'Fine for one process. You run twelve app servers behind a load balancer and they share no memory, so each one lets its own winner through.',
        },
        {
          text: 'Retrying the second request',
          explanation: 'The second request already succeeded — that is the bug. There is nothing to retry.',
        },
        {
          text: 'Putting the condition inside the write: ... WHERE seat_id=\'A5\' AND state=\'free\', then checking rowcount',
          explanation: 'Correct. Condition and write become one atomic statement, so no gap exists to lose. The loser gets rowcount 0 and shows the seat as gone.',
        },
      ],
      correct: 3,
    },
    {
      question: 'knight.is_valid_move(board, frm, to) returns True for a jump that would leave its own king in check. Where does the fix belong?',
      options: [
        {
          text: 'Pass the king\'s position into every piece\'s is_valid_move',
          explanation: 'Now every piece knows about kings. Next you pass the history for en passant, then another piece\'s moved flag for castling — the abstraction is gone.',
        },
        {
          text: 'The piece returns pseudo-moves (shape only); the Board filters them by simulating each and rejecting self-check',
          explanation: 'Correct. Shape is local to the piece, legality is a property of the whole position — so it belongs to the object that owns the whole position.',
        },
        {
          text: 'One Board.is_legal() with a switch on piece type',
          explanation: 'It produces correct results, but it deletes the polymorphism the piece hierarchy existed for and turns every variant into an edit of one giant method.',
        },
        {
          text: 'Have Knight detect pins before moving',
          explanation: 'A pin is a property of the entire position. The knight cannot compute it from its own geometry without effectively becoming the board.',
        },
      ],
      correct: 1,
    },
    {
      question: 'A 10,000-spot airport lot allocates by scanning all spots for the first free one that fits. Why is this the design failing, not merely slow?',
      options: [
        {
          text: 'It is O(n), and n is only 10,000',
          explanation: '10,000 is indeed small, so the raw number is not the issue — but the shape of the cost is.',
        },
        {
          text: 'It cannot support multiple floors',
          explanation: 'A scan works across floors perfectly well. That is not what breaks.',
        },
        {
          text: 'The cost grows exactly when the lot is fullest — peak arrival time is also when the scan walks the most taken spots before finding a free one',
          explanation: 'Correct. Load and cost peak together, which is the signature of a structure that will fall over precisely when it matters. Per-type free lists make it a pop.',
        },
        {
          text: 'It uses too much memory',
          explanation: 'A scan uses essentially none. Free lists and bitmaps are the options that add memory, and they are the fix.',
        },
      ],
      correct: 2,
    },
    {
      question: 'The parking lot needs three allocation policies (nearest lift, cheapest floor, EV-first). Which design?',
      options: [
        {
          text: 'One AllocationStrategy interface with three implementations, injected into ParkingLot',
          explanation: 'Correct. One axis genuinely varies, three real implementations exist today, and the lot never learns any of them. That is when an interface is earned.',
        },
        {
          text: 'Three subclasses of ParkingLot',
          explanation: 'is-a abuse: an EV-first lot is not a kind of lot, it is a lot with a policy. You also lose the ability to change policy at runtime.',
        },
        {
          text: 'An if policy == ... chain inside park()',
          explanation: 'Every new policy edits the one method everything depends on, and ParkingLot ends up knowing all three policies — the coupling Strategy exists to remove.',
        },
        {
          text: 'A config flag read from a file inside the allocation loop',
          explanation: 'Configuration is not the question. Where the behaviour lives is — and this still leaves all three behaviours inside the lot.',
        },
      ],
      correct: 0,
    },
    {
      question: 'Greedy debt simplification (biggest debtor pays biggest creditor) on 5 people with non-zero balances. What can you honestly claim?',
      options: [
        {
          text: 'It always produces the provably minimum number of transfers',
          explanation: 'The trap. The true minimum requires finding subsets that sum to zero, which is NP-hard. Claiming optimality here is the answer interviewers are fishing for.',
        },
        {
          text: 'At most 4 (n − 1) transfers, because each step zeroes at least one person — good, but not guaranteed minimal',
          explanation: 'Correct, and it is the honest sentence: a real bound, plus an explicit admission that greedy is a heuristic, not an optimum.',
        },
        {
          text: 'Exactly 4 transfers',
          explanation: 'Often fewer. Any subgroup whose balances net to zero collapses early, so n − 1 is an upper bound, not a count.',
        },
        {
          text: 'It can loop forever when balances do not divide evenly',
          explanation: 'Each iteration removes at least one person from consideration, so it terminates in at most n steps regardless of the amounts.',
        },
      ],
      correct: 1,
    },
